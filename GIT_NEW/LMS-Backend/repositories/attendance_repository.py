...
# Durable trace of attendance write failures — console output from run_8001.bat
# is not captured to disk, so a swallowed/printed error leaves no record.
...
def insert_check_in(card_no: str, empcode: str, *,
    ...
    try:
        ...
        # NOTE: a MERGE failure here is fatal (falls through to the outer except
        # below) — it must never be swallowed, or the app reports "success" while
        # nothing is written.
        params = {
            "empcode": empcode,
            "card_no": card_no,
        ...
        _logger.error(f"[CHECK_IN] card={card_no} empcode={empcode}: {e}")
...
def update_check_out(record_id: int, entry_time: str, card_no: str = None,
    ...
    try:
        ...
        conn.rollback()
        print(f"[CHECK_OUT] Fatal error for record_id={record_id}, card={card_no}: {e}")
        _logger.error(f"[CHECK_OUT] record_id={record_id} card={card_no}: {e}")
        return {"status": "error", "message": str(e)}
    finally:
        cursor.close()
        conn.close()
