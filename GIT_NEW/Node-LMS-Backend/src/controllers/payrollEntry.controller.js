...
import {
  listOpenPeriods, listPayrollEmployees, recoveryTypes,
...
// GET /payroll-entry/employees — employees eligible for payroll entry in a period
export const getPayrollEmployees = async (req, res, next) => {
  try {
    ...
    res.json({ items: await listPayrollEmployees(company, period) });
  } catch (err) {
    next(err);
  }
};
