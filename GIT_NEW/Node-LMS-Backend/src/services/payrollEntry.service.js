...
/**
 * Employees eligible for payroll entry — those in HR_EMP_MASTER with active
 * status for the given company. Mirrors FastAPI's list_payroll_employees.
 */
export const listPayrollEmployees = async (compc, period = null) => {
  let connection;
  try {
    connection = await getDirectConnection();
    const result = await connection.execute(
      `SELECT DISTINCT
              h.EMPCODE, h.NAME, h.BASIC, h.UNIT_ID, h.LOCATION,
              NVL(h.STATUS, 'A') AS emp_status
         FROM HR_EMP_MASTER h
        WHERE h.UNIT_ID = :u
          AND UPPER(NVL(h.STATUS, 'A')) IN ('A', 'E')
        ORDER BY h.NAME`,
      { u: toInt(compc) },
      { outFormat: OUT_ARRAY }
    );
