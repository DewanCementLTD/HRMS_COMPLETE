...
export const getMonthlyAllowances = async (req, res, next) => {
  try {
    const { admin_card_no, compc, brnch, empcode } = res.locals.validated.query;
    const unitId = await resolveCompany(admin_card_no, compc);
    const branch = await resolveBranch(admin_card_no, compc, brnch);
    res.json({ items: await listMonthlyAllowances(unitId, null, empcode, branch) });
  } catch (err) {
    next(err);
  }
};
...
export const postMonthlyAllowance = async (req, res, next) => {
  try {
    ...
    const result = await upsertMonthlyAllowance(await resolveCompany(admin_card_no, compc), empcode, allowance_id, amount, ot_hour, remarks, { usr: PAYROLL_USR });
    checked(res, result);
  } catch (err) {
    next(err);
  }
...
export const delMonthlyAllowance = async (req, res, next) => {
  try {
    const { admin_card_no, empcode, allowance_id, compc } = res.locals.validated.query;
    checked(res, await deleteMonthlyAllowance(await resolveCompany(admin_card_no, compc), empcode, allowance_id));
  } catch (err) {
    next(err);
  }
};
...
export const getMonthlyDeductions = async (req, res, next) => {
  try {
    ...
    const unitId = await resolveCompany(admin_card_no, compc);
    const branch = await resolveBranch(admin_card_no, compc, brnch);
    res.json({ items: await listMonthlyDeductions(unitId, null, empcode, branch) });
  } catch (err) {
    next(err);
  }
};
...
export const postMonthlyDeduction = async (req, res, next) => {
  try {
    ...
    const result = await upsertMonthlyDeduction(await resolveCompany(admin_card_no, compc), empcode, deduction_id, amount, remarks, { usr: PAYROLL_USR });
    checked(res, result);
  } catch (err) {
    next(err);
  }
...
export const delMonthlyDeduction = async (req, res, next) => {
  try {
    const { admin_card_no, empcode, deduction_id, compc } = res.locals.validated.query;
    checked(res, await deleteMonthlyDeduction(await resolveCompany(admin_card_no, compc), empcode, deduction_id));
  } catch (err) {
    next(err);
  }
};
...
export const getAbsentDays = async (req, res, next) => {
  try {
    ...
    const unitId = await resolveCompany(admin_card_no, compc);
    const branch = await resolveBranch(admin_card_no, compc, brnch);
    res.json({ items: await listAbsentDays(unitId, null, empcode, branch) });
  } catch (err) {
    next(err);
  }
};
...
export const getEmployeeAbsentDays = async (req, res, next) => {
  try {
    const { admin_card_no, empcode, compc } = res.locals.validated.query;
    res.json(await getEmployeeAbsent(await resolveCompany(admin_card_no, compc), empcode));
  } catch (err) {
    next(err);
  }
};
...
export const postAbsentDays = async (req, res, next) => {
  try {
    ...
    checked(res, await setAbsentDays(await resolveCompany(admin_card_no, compc), empcode, absent_days, { usr: PAYROLL_USR }));
  } catch (err) {
    next(err);
  }
};
...
export const delAbsentDays = async (req, res, next) => {
  try {
    const { admin_card_no, empcode, compc } = res.locals.validated.query;
    checked(res, await deleteAbsentDays(await resolveCompany(admin_card_no, compc), empcode));
  } catch (err) {
    next(err);
  }
};
